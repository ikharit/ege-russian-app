...
// Extract entries by parsing lines
...
// Foreign root indicators (suffixes and roots)
const foreignSuffixes = ['ция', 'тет', 'м', 'нт', 'в', 'зм', 'тика', 'изм', 'ист', 'фик', 'вый', 'вое', 'ное', 'ный', 'ние', 'тие', 'ство', 'ость', 'ание', 'ение', 'ован', 'ирован', 'изир', 'лог', 'граф', 'метр', 'нез', 'тез', 'сфер', 'под', 'груп', 'зит', 'зиц', 'хрон', 'хор', 'тракт', 'виз', 'мил', 'сил', 'пил', 'тил', 'кил', 'фил', 'фон', 'хол', 'тол', 'кол', 'пол', 'вол', 'шор', 'жор', 'чер', 'мер', 'тер', 'пер', 'нер', 'вер', 'сер', 'дер', 'жер', 'чер', 'шер', 'гр', 'пр', 'тр', 'др', 'кр', 'мр', 'вр', 'ср', 'зр', 'жр', 'чр', 'шр', 'щр'];

const knownForeignRoots = ['территор', 'уровен', 'монолог', 'вазелин', 'навигац', 'коррект', 'утоп', 'гениал', 'коллекцион', 'паломничеств', 'карниз', 'митинг', 'трамвай', 'павильон', 'портрет', 'либерал', 'индивид', 'дефицит', 'манифест', 'пессимист', 'брошюр', 'оранжер', 'бетон', 'анчоус', 'президент', 'терап', 'конференц', 'информ', 'организац', 'акварель', 'суверенитет', 'абитуриент', 'меридиан', 'новелл', 'конкурент', 'энциклопед', 'привилег', 'цивилизац', 'цитат', 'реценз', 'компан', 'томительн', 'петербургск', 'велосипед', 'циклическ', 'цилиндр', 'декларац', 'экзаменатор', 'пессимизм', 'универ', 'аннотац', 'трафарет', 'полемичн', 'виртуальн', 'стилист', 'престиж', 'ароматн', 'колоссальн', 'деликатес', 'офицер', 'менталитет', 'мажор', 'циклон', 'социальн', 'демократ', 'уникальн', 'героическ', 'тревог', 'укреплен', 'демонстрир', 'фигурир', 'тростник', 'тренир', 'продолжен', 'абонемент', 'конфорк', 'громад', 'фантаст', 'традиц', 'тенденциоз', 'вертикальн', 'анализ', 'результат', 'конкрет', 'системат', 'лимит', 'ориентиров', 'классицист', 'пейзаж', 'программ', 'корифей', 'цистерн', 'панцир', 'аккомпанир', 'аналогичн', 'конфликт', 'инженер', 'впечатл', 'трущоб', 'моратор', 'офицерск', 'акварельн', 'веществен', 'юнош', 'сонет', 'жираф', 'мажорн', 'симптом', 'минерал', 'робот', 'робаст', 'робот', 'робот', 'робот', 'робот', 'робот', 'робот', 'робот', 'робот', 'робот', 'робот', 'робот', 'робот', 'робот', 'робот', 'робот', 'робот', 'робот', 'робот'];
...
function classify(word) {
  ...
  // Check known foreign roots first
  ...
  // Try to extract a reasonable root (remove common suffixes)
  let root = lower;
  ...
  // Also remove prefixes for root extraction
  ...
  // Clean root: keep only meaningful part (at least 3 chars)
...
for (const word of words) {
  ...
  newEntries[word] = result;
  classified[result.type]++;
  
  // Add to existing dict if not present
...
// Save new entries for manual review
fs.writeFileSync('ege-russian-app/auto_classified.json', JSON.stringify(newEntries, null, 2), 'utf-8');
console.log('Saved to auto_classified.json');

// Now regenerate rootDictionary section in the file
...
const newContent = content.replace(/const rootDictionary = \{[\s\S]*?\};/, newDictSection);
