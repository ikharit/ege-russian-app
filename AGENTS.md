...
- **⚠️ Важно:** В `grammar.ts` `lesson-gram-12-2` есть вопрос `q12-16` "построен.." с вариантами `построен/построенн` — формат вопроса кривой (это не ЕГЭ-формат). Нужно либо заменить на вставку пропущенной буквы, либо удалить. Все агенты должны проверять задания через этот Excel перед внесением изменений.
