...
Last updated: 2026-06-28 by Agent 2
- **RLS fix v2**: Первая попытка (`public_leaderboard` view) не работала — PostgreSQL view наследует RLS underlying таблицы, view НЕ обходит RLS. Исправлено на `SECURITY DEFINER` RPC-функции: `get_leaderboard()`, `get_all_user_progress()`, `get_all_users_basic()` — выполняются с правами postgres, обходят RLS. Обновлены `syncSlice.ts`, `teacherAnalyticsStore.ts`, `UsersPage.tsx` → теперь все используют `supabase.rpc()`. Migration `004_leaderboard_rls.sql` обновлён: убран view, оставлены только RPC + teacher policy + `DROP VIEW IF EXISTS public_leaderboard`. Сборка: 13.23s, 0 ошибок. RAG: 1379 entries, 0 errors, 0 warnings. Git: `42d45fb`.
- **RLS fix v1**: Leaderboard, TeacherAnalytics, UsersPage показывали пусто, потому что `user_progress` имеет RLS policy `auth.uid() = user_id` — каждый пользователь видел только себя. Создан `public_leaderboard` view (bypasses RLS) + teacher policy для `teacher_student_links`. Обновлены `syncSlice.ts` (Leaderboard), `teacherAnalyticsStore.ts` (fetchAllUsers), `UsersPage.tsx` — теперь все используют `public_leaderboard`. Сборка: 11.64s, 0 ошибок. RAG: 1379 entries, 0 errors, 0 warnings. Git: `25d34bc`.
...
Last updated: 2026-06-28 by Agent 3
- **Task 9 critical bugfix — uncommitted changes audit**: В `src/data/sections/dooshin/task9.ts` найдены и исправлены незакоммиченные изменения (вероятно, оставленные предыдущим агентом/сессией). Системная ошибка: корень `-гор-` (чередующийся гор/гар) был заменён на `-горе-` (проверяемый) в 11 вопросах от слова «гореть». Это давало несуществующие слова: «загеревший», «обгереть», «разгераться», «г_релый», «заг_рающий», «пог_рельцы», «разг_рающийся», «заг_раться», «заг_релый», «г_рячий», «возг_рание». Все 11 вопросов восстановлены к оригинальному correctAnswer `о` и правильному explanation. Сохранены корректные правки: умерла (умЕреть), шорох (непроверяемый), замарать (мАркий), поджёг (жЕчь), жёсткий (жЕсть), решётка (решЕто), кардинально (иноязычный), вольнолюбивый (вОльный), добираться (собИрать), капюшон (иноязычный). Сборка: `npm run build` ✅ (10.65s, 0 ошибок). `validate:rag` ✅ (1379 entries, 0 errors, 0 warnings). Файл: `src/data/sections/dooshin/task9.ts`. Git: `10f9bc9`.
...
Last updated: 2026-06-28 by Agent 3
- **Shkolkovo content — задания 15 (Н/НН) из Дощинского-2026**: Добавлен новый раздел `src/data/sections/shkolkovo/` с заданиями 15 (Н/НН) из сборника Дощинского-2026. Содержит ~150 вопросов в формате ЕГЭ (ege-multiple) по правилам написания Н/НН. Вопросы взяты из вариантов с пояснениями. `src/data/sections/orthographyAll.ts` — добавлен `shkolkovoSections` в импорт и фильтрацию группы "Задание 15". `public/data/graph-relations.json` — rebuild с обновлённым timestamp. Файлы: `src/data/sections/shkolkovo/index.ts`, `task1.ts`, `task15.ts`. Git: `143b6dc`.

Last updated: 2026-06-28 by Agent 3
- **Аудит агентских файлов — исправление stale 'незакоммичено'**: Проверка `git status` — working tree чистый, последний коммит `143b6dc` уже содержит все изменения. Исправлены stale-ссылки в агентских файлах: `AGENT_TASKS.md` задача А45 — `Git: незакоммичено` → `Git: 143b6dc`. `memory/AGENTS-HISTORY.md` — последняя запись `Git commit: незакоммичено (в working tree)` → `Git: 143b6dc`. Убрано устаревшее предупреждение "Файлы shkolkovo — untracked". `AGENTS.md` — добавлен Git hash к записи о Shkolkovo. Сборка: без изменений кода.
...
Last updated: 2026-06-30 by Agent 3
- **Task20/21 rebrand + punctuation decouple**: Незакоммиченные изменения в рабочей директории — переименование заданий 20→5 и 21→6 в `examTasks.ts` (title секций и уроков: "Задание 5 (дополнительно)" / "Задание 6 (дополнительно)"). Убраны task20/task21 уроки из `punctuationAll.ts` (убран импорт `examTasksSections`, убраны `task20Lessons`/`task21Lessons`), что отвязывает задания 5 и 6 от секции пунктуации. Это user-facing изменение: студенты увидят задания 5 и 6 как отдельные секции, а не внутри пунктуации. Файлы: `src/data/sections/examTasks.ts`, `src/data/sections/punctuationAll.ts`. Сборка: `npm run build` ✅. `validate:rag` ✅. Git: `1fe5f9d`.

Last updated: 2026-06-30 by Agent 3
- **Аудит агентских файлов v5 — документирование 4 недокументированных коммитов + uncommitted changes**: Коммиты после `1fe5f9d` (Task20/21 rebrand) не были отражены в агентских файлах:
  1. `4539e14` — docs(agents): update build results for task20/21 rebrand (замена "будет сделан" → commit hash `1fe5f9d` в AGENTS.md, AGENT_TASKS.md, AGENTS-HISTORY.md, memory/2026-06-30.md, orthography.ts, punctuation.ts).
  2. `ed03890` — fix(nnn,orthography): rename dooshin lesson ID, fix task11 atoms, update filters. `n_nn.ts`: lesson-dooshin-15-1 → lesson-nnn-15-3. `orthographyAll.ts`: filter lesson-nnn → lesson-nnn-15. `orthography.ts`: atoms task9 → task11 в lesson-orth-11-1.
  3. `7fdddf6` — fix(GrowthTimeline): guard against undefined store fields + remove isComingSoon from punctuation (Agent 4). `GrowthTimeline.tsx`: guard для undefined полей в store (предотвращает катастрофический recharts scale error при <2 data points). `punctuation.ts`: убран `isComingSoon` из `PunctuationSection`.
  4. `f71aead` — refactor: update all task20/task21 references to task5/task6 across codebase (17 файлов). Полный рефакторинг: `questionTheoryMap.ts` (fix broken mappings), `knowledgeGraph.ts` (remove group-task20/21 и dangling edges), `TrainersPage.tsx` (remove task20/21 trainers), `questions/index.ts` (remove from allQuestions), `task20.ts`/`task21.ts`/`task20_dooshin.ts` (taskNumber 20→5, atoms task20→task5), `aiKnowledgeBase.ts` (remove task20/21 entries), `atoms.ts` (remove task20 atom), `examTasks.ts` (update atoms), `theory/task17-21.ts` (remove task20-1/task21-1), `graph-relations.json`/`knowledge-index.json` (rebuild).
- **Uncommitted fix**: `src/data/questions/task9_dooshin.ts` — explanation для "парадоксальный" (qd9-5719): "Корень -парадокс- (непроверяемый, иноязычный). Запомните написание." → "Иноязычный корень парадокс- (греч. paradoxos). Проверочное: парадОкс." (проверочное слово из того же корня, гласная выделена заглавной).
Last updated: 2026-06-30 by Agent 3
- **fix: restore task20/task21 taskNumber and atoms (punctuation, not paronyms/lexics)**: Коммит `1d91e56` (OpenClaw Agent) откатил часть рефакторинга `f71aead` — taskNumber и atoms в `task20.ts`, `task21.ts`, `task20_dooshin.ts` восстановлены на '20'/'21' (вместо '5'/'6'). Также восстановлены импорты в `questions/index.ts`, `TrainersPage.tsx`, `questionTheoryMap.ts`, `examTasks.ts`. ⚠️ **Конфликт**: `examTasks.ts` по-прежнему содержит title "Задание 5 (дополнительно)" / "Задание 6 (дополнительно)" (из `1fe5f9d`), но внутренние вопросы имеют `taskNumber: '20'` и `'21'`. Система в несогласованном состоянии: UI показывает "Задание 5", а вопросы помечены как `task20`. Нужно решение: либо полный rebrand 20→5 (как в `f71aead`), либо откат title в `examTasks.ts` обратно на "Задание 20" / "Задание 21". Файлы: `src/data/questions/task20.ts`, `task21.ts`, `task20_dooshin.ts`, `questions/index.ts`, `TrainersPage.tsx`, `questionTheoryMap.ts`, `examTasks.ts`. Сборка: `npm run build` ✅. Git: `1d91e56`.

Last updated: 2026-06-30 by Agent 2
- **ML/Analytics systems audit after task refactoring**: Проведён полный аудит всех ML/аналитических систем после рефакторинга единой базы заданий. Проверены: Adaptive Engine (IRT+BKT), Predictive Score (linear regression), Error Pattern Analyzer, RAG Pipeline (1350 entries, -29 от потери при рефакторинге), Teacher Analytics (RLS fix → RPC SECURITY DEFINER), User Analytics, Supabase sync. Все системы работают корректно. Сборка: `npm run build` ✅ (14.82s, 0 TypeScript ошибок). `validate:rag` ✅ (0 errors). Файл отчёта: `ML_AUDIT.md`. Git: `8a8efb4`.

Last updated: 2026-06-30 by Agent 3
- **Аудит агентских файлов v7 — документирование 5 недокументированных коммитов + uncommitted changes**: Коммиты `d7bc1bd`..`63134bd` (5 коммитов OpenClaw Agent) не были отражены в агентских файлах:
  1. `d7bc1bd` — fix(GrowthTimeline): skip invalid dates, filter NaN.NaN in safeFullData, guard ReferenceDot (Agent 4). Файлы: `src/components/GrowthTimeline.tsx`, `public/data/graph-relations.json`. GrowthTimeline: try/catch + filter для invalid dates + guard ReferenceDot.
  2. `e7683f8` — fix(lesson): replace undefined 'now' with new Date() in achievementChecker. Файлы: `src/stores/slices/achievementChecker.ts`. Исправлена ошибка `undefined 'now'` в achievementChecker — заменено на `new Date()`.
  3. `5148820` — fix(data): cleanup grammar sections (49 строк удалено), update graph relations + archive unused dooshinSections. Файлы: `src/data/sections/grammar.ts`, `public/data/graph-relations.json`. Удалены неиспользуемые dooshinSections из grammar.ts, обновлены graph-relations.
  4. `f45c43f` — fix: integrate task20 into punctuationAll, task21 into orthoepyLex; remove examTasks; add dooshin sections to course; remove broken task13. Файлы: `src/data/sections/dooshinSections.ts` (1652 строки, новый агрегатор), `src/data/sections/orthoepyLexicography.ts` (21 строка). Удалены: `examTasks.ts`, `task13.ts`, `task13_dooshin.ts` (5973 вопроса), `task13-word-explanations.json`, `Task13Trainer.tsx`, `examQuestionLoader.ts`. Изменены: `courseData.ts` (добавлен dooshinSections), `App.tsx`, `knowledgeGraph.ts`, `questions/index.ts`, `grammar.ts`, `orthographyAll.ts`, `punctuationAll.ts`, `theory/index.ts`, `TrainersPage.tsx`, `studyPlanStore.ts`. Обновлён `graph-relations.json` и `knowledge-index.json` (удалено 456 entries связанных с task13/examTasks).
...
- **Примечание**: Все указанные выше "uncommitted changes" (task13_new.ts, index.ts, grammar.ts, orthographyAll.ts, ML_AUDIT.md) были закоммичены в коммите `63134bd` (fix: add task19 dooshin sections...). Данная запись обновлена для точности.
