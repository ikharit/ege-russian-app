...
- **Зачем:** Чтобы ВСЕ агенты могли проверять задания перед правкой. JSON читается программно, Excel — для ручной проверки.
- **Git commit:** `feat/task-registry-system`
- **⚠️ Важно:** Любой агент, получивший задание 9-12, должен сначала прочитать `TASK_REGISTRY.md` и при необходимости использовать `task_registry.json` для проверки ответов.
