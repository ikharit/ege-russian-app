...
import { task5QuestionsById } from '../data/questions/task5'
import { BaseTrainer } from '../components/BaseTrainer'

export function Task5Trainer() {
  const questions = Object.values(task5QuestionsById)
  ...
  return (
    <BaseTrainer
      ...
      getQuestionText={(q) => q.sentences.map((s) => s.text).join(' / ')}
      ...
      renderPrompt={() => (
        <p className="text-gray-700 text-sm leading-relaxed">
          В одном из приведённых ниже предложений <strong>неверно</strong>{' '}
          употреблено выделенное слово. Найдите это предложение.
        </p>
      )}
      renderQuestion={({
        question,
        ...
        return (
          <div className="w-full space-y-2">
            {question.sentences.map((sentence) => {
              ...
              const showCorrect =
                answerState !== 'idle' && isErrorSentence
              const showWrong =
                answerState !== 'idle' && isSelected && !isErrorSentence
              const showMissed =
                answerState !== 'idle' &&
                !isSelected &&
                isErrorSentence
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 text-sm text-gray-700 leading-relaxed">
                      {sentence.text.split(/\*\*(.+?)\*\*/g).map((part, i) =>
                        i % 2 === 1 ? (
